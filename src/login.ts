...
const login = fn<{username: string; password: string; resource: string}, void>(
  async (args, _, {req, res}) => {
    ...
    const {data, errors} = await userAuthenticate.execute(args)
    ...
    if (data) {
      setAuthentication(data.userId, res)

      const userDataToken = await newUserDataToken(data.userId)
